...
- [0. ゴール定義](#0-ゴール定義)
- [1. アーキテクチャ（UIとRunner分離）](#1-アーキテクチャuiとrunner分離)
- [2. フォルダ構成（初期）](#2-フォルダ構成初期)
- [3. UI（PySide6）仕様（最小）](#3-uipyside6仕様最小)
- [4. Runner（実行エンジン）仕様](#4-runner実行エンジン仕様)
- [5. Actions（原子的操作）実装順](#5-actions原子的操作実装順)
- [6. シナリオ（MVPは2本）](#6-シナリオmvpは2本)
- [7. データ（mock）設計](#7-データmock設計)
- [8. ログ/証跡（必須）](#8-ログ証跡必須)
- [9. 安全装置（最低限）](#9-安全装置最低限)
- [10. 実装タスク（順番このまま）](#10-実装タスク順番このまま)
- [11. その後の拡張ポイント](#11-その後の拡張ポイント)

関連ドキュメント：`docs/README.md` / `docs/api.md` / `docs/mock.md`
...
> 参考：Runnerに渡す「Primitive(Command)配列」案は `docs/api.md` を正本にする。
